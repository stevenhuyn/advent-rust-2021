...
fn solve_p2() {
    ...
    let octopus_count = (grid.len() * grid[0].len()) as i32;

    for i in 1.. {
        if iterate(&mut grid) == octopus_count {
            println!("First all flash: {}", i);
            break;
        }
    }
}

fn iterate(grid: &mut Vec<Vec<usize>>) -> i32 {
